"""TaskPlanner agent for request analysis, reformulation, and clarification flow.

The planner is part of the active LangGraph pipeline and runs before
prepare_generation_context when `PLANNER_ENABLED` is enabled. It analyzes the
current user turn, can request clarification, and can decide whether a
follow-up message should refine existing code or start a fresh generation.
"""
