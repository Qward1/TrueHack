...
def build_lowcode_validation_harness(
    ...
    harness = (
        ...
        "    local keys = {}\n"
        "    for key, _ in pairs(value) do\n"
        "        keys[#keys + 1] = { original = key, rendered = tostring(key) }\n"
        "    end\n"
        ...
        "        parts[#parts + 1] = '\"' .. _json_escape_runtime(key_info.rendered) .. '\":' .. _serialize_runtime_result(value[key_info.original], depth + 1, seen)\n"
        "    end\n"
        "    seen[value] = nil\n"
        "    return '{' .. table.concat(parts, ',') .. '}'\n"
        "end\n"
